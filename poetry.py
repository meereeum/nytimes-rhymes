    ...
    def get_rhymes(self, word):
        ...
        try:
            return [ rhyme for pronunciation in self.d_phonemes[word]
                    for rhyme in self.d_rhymes[self.rhymable_sound(pronunciation)]
                     if rhyme != word ]
        except(KeyError):
            return []
...
class Poem():
    def __init__(self, text, d_phonemes, stress_matters=True, ignore_plural=True,
                 print_rhymable=False, print_rhymes=False,
                 print_individual=False, print_combined=True):
    ...
    def partition(self, idxs):
        ...
        if not idxs:
            return [self.l_words]
        idxs.sort()
        # avoid duplicating slicepoints at beginning or end of word list
        idxs = idxs[ (idxs[0]==0) : len(idxs) - (idxs[-1] == len(self)) ]
        to_slice = zip([0]+idxs, idxs+[len(self)])
        return [ self.l_words[i:j] for (i,j) in to_slice ]


    def find_rhyming_idxs(self):
        """Identify indices corresponding to true rhymes (non-self) among potential rhymes"""
        ...
        for rhyme in self.potential_rhymes:
            ...
            nonredundant_idxs = [ [word[0] for word in grouped]
                                for _,grouped in itertools.groupby(
                                        ...
                                        key=lambda x: x[1]) ]
            #print idxd_rhymes
            #print nonredundant_idxs
            if len(nonredundant_idxs) > 1:
                # list of lists with all combinations of nonredundant rhymes
                self.rhyming_idxs.append( [idxs for idxs in
                                           itertools.product(*nonredundant_idxs)] )
                #print [[(alphabet[alph_i], idx) for idx in idxs] for idxs in itertools.product(*nonredundant_idxs)]
                alph_i+=1


    def combine_idxs(self):
        """Generate all combinations of rhyming indices that ignore self-rhymes"""
